 ...
 *   0. Part-time max-hours    — hard block (part-time staff on shifts > 6 h)
 ...
 *
 * Step 0 comes first because it is a pure-field check (no DB query) and
 * blocks early for part-timers, avoiding unnecessary DB lookups.
 */
...
async function evaluateCandidate(
  ...
  // Step 0: Crew position check — pure field check, no DB needed
  // Only drivers can fill driver slots; only medics/EMTs/paramedics can fill attendant slots.
  if (slot.crew_position === 'driver' && candidate.role !== 'driver') {
    return {
      staff_id: candidate.staff_id,
      full_name: candidate.full_name,
      role: candidate.role,
      eligible: false,
      hard_blocked: true,
      block_reason: `Staff role '${candidate.role}' cannot fill a driver slot`,
      consecutive_days_flag: false,
      consecutive_days_count: 0,
    };
  }
  if (slot.crew_position === 'attendant' && candidate.role === 'driver') {
    return {
      staff_id: candidate.staff_id,
      full_name: candidate.full_name,
      role: candidate.role,
      eligible: false,
      hard_blocked: true,
      block_reason: 'Driver cannot fill an attendant slot',
      consecutive_days_flag: false,
      consecutive_days_count: 0,
    };
  }

  // Step 0b: Part-time max-hours check — pure field check, no DB needed
  if (candidate.employment_type === 'part_time' && slotDuration > 360) {
    return {
      staff_id: candidate.staff_id,
      full_name: candidate.full_name,
      role: candidate.role,
      eligible: false,
      hard_blocked: true,
      block_reason: `Part-time staff cannot work shifts exceeding 6 hours (${(slotDuration / 60).toFixed(0)}h shift)`,
      consecutive_days_flag: false,
      consecutive_days_count: 0,
    };
  }
  ...
  const blockedByLeave = await isBlockedByLeaveOrAvailability(
    candidate.staff_id, rosterDate, slotStart, slotEnd
  );
  if (blockedByLeave) {
    return {
      staff_id: candidate.staff_id,
  ...
  const dailyMinutes = await getDailyScheduledMinutes(candidate.staff_id, rosterDate, slot.slot_id);
  const dailyLimit = candidate.employment_type === 'part_time' ? 360 : 720;
  if (dailyMinutes + slotDuration > dailyLimit) {
    return {
      staff_id: candidate.staff_id,
      full_name: candidate.full_name,
      role: candidate.role,
      eligible: false,
      hard_blocked: true,
      block_reason: `Would exceed ${dailyLimit / 60}h daily limit (${((dailyMinutes + slotDuration) / 60).toFixed(1)}h)`,
      consecutive_days_flag: false,
      consecutive_days_count: 0,
    };
  }
  ...
  const consecutiveFlag = consecutiveDays >= 6;
 ...
 * All candidates are evaluated concurrently for maximum throughput.
 */
export async function filterCandidates(
  ...
  return Promise.all(
    candidates.map((c) =>
      evaluateCandidate(c, slot, rosterDate, slotStart, slotEnd, slotDuration)
    )
  );
}
