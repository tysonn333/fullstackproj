...
/**
 * Fetch a slot with its roster date — reused from assignment.routes logic.
 */
async function fetchSlotWithDate(slotId: number): Promise<{ slot: Record<string, unknown>; rosterDate: string } | null> {
 ...
 * Undoes a reassignment by updating the existing audit log entry (not creating a new one).
 * Body: { reason?: string }
 */
router.post('/:id/undo', async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (!slot_id || !previous_staff_id || !roster_id) {
      res.status(422).json({ error: 'Audit log entry is missing required details (slot_id, previous_staff_id, roster_id)' });
      return;
    }
    ...
    // Validate the new staff (previous_staff_id) is eligible
    ...
    if (!candidate || candidate.hard_blocked) {
      res.status(422).json({
        error: `Cannot undo: staff #${previous_staff_id} is not eligible: ${candidate?.block_reason ?? 'not found in eligible pool'}`,
      });
      return;
    }
    ...
    // Update the existing audit log entry with undone status (no new audit entry created)
    const undoDetails = {
      ...details,
      undone: true,
