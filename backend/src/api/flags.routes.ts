 ...
 */
router.put('/:id/resolve', requireRole('admin', 'ops_director'), async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
 ...
 * Body: { reason? }
 */
router.put('/:id/dismiss', requireRole('admin', 'ops_director'), async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    const flagId = parseInt(req.params.id, 10);
