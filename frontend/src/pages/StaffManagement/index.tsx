...
export const StaffManagement: React.FC = () => {
  ...
  const handleSave = (savedStaff: Staff, credentials?: { email: string; password: string }) => {
    setStaff((prev) => {
  ...
  const handleCreateAccount = async (staffMember: Staff) => {
    setAccountLoading(staffMember.id);
    setAccountResult(null);
    try {
      const result = await staffApi.createAccount(staffMember.id);
      setAccountResult(result);
      success('Account created', `Login credentials generated for ${staffMember.name}.`);
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : 'Failed to create account';
      toastError('Account creation failed', msg);
    } finally {
          ...
          <div className="w-96 flex-shrink-0">
            <div className="card">
              <div className="card-header">
                ...
                <button
                  onClick={() => { setDrawerMode(null); setAccountResult(null); }}
              ...
              <div className="card-body">
                ...
                {/* Account creation for existing staff */}
                {drawerMode !== 'create' && selectedStaff && selectedStaff.email && (
                  <div className="mt-4 pt-4 border-t border-gray-100">
                    {accountResult ? (
                      <div className="p-3 bg-green-50 border border-green-200 rounded-lg text-sm mb-3">
                        <p className="font-semibold text-green-800 mb-1">Account Created</p>
                        <p className="text-green-700">Email: <span className="font-mono">{accountResult.email}</span></p>
                        <p className="text-green-700">Password: <span className="font-mono">{accountResult.password}</span></p>
                        <p className="text-xs text-green-600 mt-1">Share these credentials with the staff member.</p>
                      </div>
                    ) : (
                      <button
                        onClick={() => handleCreateAccount(selectedStaff)}
                        disabled={accountLoading === selectedStaff.id}
                        className="btn-secondary w-full text-sm"
                      >
                        {accountLoading === selectedStaff.id ? (
